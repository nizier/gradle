...
public class TaskController {
    ...
    @Autowired
    DM_TS_CL_ORGMapper dm_ts_cl_orgMapper;
    @Autowired
    DeleteDataController deleteDataController;
    @Autowired
    UNIT_PRICE_RANGEMapper unit_price_rangeMapper;
    ...
    public String startTask(String SearchTime) {
        if(mappers.isEmpty()){
            ...
            mappers.put("DM_TS_CL_ORG",dm_ts_cl_orgMapper);
            mappers.put("UNIT_PRICE_RANGE",unit_price_rangeMapper);

        }
        LoggerService.Instance().setLoggerMapper(check_logMapper);
        new Thread(new CheckService(mappers,SearchTime,deleteDataController)).start();
        return "start task...";
    }
}
