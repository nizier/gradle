...
@Component
@Mapper
public interface DCL_IO_DECL_HISTORYMapper {
    @Insert({
        "insert into DCL_IO_DECL_HISTORY (DECL_NO, TRADE_MODE_CODE, ",
        "CONTRACT_NO, MARK_NO, ",
    ...
    int insert(DCL_IO_DECL_HISTORY record);

    @Delete("DELETE FROM DCL_IO_DECL_HISTORY WHERE DECL_NO=#{DECL_NO,jdbcType=VARCHAR}")
    int deleteByDECL_NO(String DECL_NO);

    @InsertProvider(type=DCL_IO_DECL_HISTORYSqlProvider.class, method="insertSelective")
    int insertSelective(DCL_IO_DECL_HISTORY record);

    //T根据报检单号，查询报检信息
    @Select("SELECT * FROM DCL_IO_DECL_HISTORY WHERE DECL_NO=#{DECL_NO,jdbcType=VARCHAR}")
    @Results({
    ...
    DCL_IO_DECL_TASK getDclIoDeclTaskHistoryByDeclNo(@Param("DECL_NO") String DECL_NO);

    @Delete({"DELETE FROM DCL_IO_DECL_HISTORY WHERE \"OPER_TIME_TJ\"<=to_date(#{strBeforeDate,jdbcType=VARCHAR}, 'yyyy-mm-dd')"})
    int deleteIO_DECLhistoryData(String strBeforeDate);
}