...
@Component
@Mapper
public interface DCL_IO_DECL_GOODS_TASKMapper {
    @Insert({
        "insert into DCL_IO_DECL_GOODS_TASK (GOODS_ID, DECL_NO, ",
    ...
    int insert(DCL_IO_DECL_GOODS_TASK record);

    @InsertProvider(type=DCL_IO_DECL_GOODS_TASKSqlProvider.class, method="insertSelective")
    int insertSelective(DCL_IO_DECL_GOODS_TASK record);

    //TODO 过滤条件后期修改
    @Select("SELECT * FROM DCL_IO_DECL_GOODS WHERE \"OPER_TIME_TJ\">=to_date(#{QueryTime,jdbcType=VARCHAR}, 'yyyy-mm-dd') AND \"OPER_TIME_TJ\"<to_date(#{QueryEndTime,jdbcType=VARCHAR}, 'yyyy-mm-dd') ")
    ...
    List<DCL_IO_DECL_GOODS_TASK> listDatas(@Param("QueryTime") String QueryTime,@Param("QueryEndTime")String QueryEndTime);

    // OPER_TIME_TJ 及 取1W条数据都是临时字段，后期更改
    @Select({"SELECT DECL_NO,GOODS_NO,MAX(OPER_TIME_TJ) AS \"OPER_TIME_TJ\" FROM DCL_IO_DECL_GOODS WHERE ROWNUM<10000 GROUP BY DECL_NO,GOODS_NO"})
    @Results({
    ...
    List<DCL_IO_DECL_GOODS_TEMP> listTempDatas();

    //根据报检单号、货物序号及OPER_TIME_TJ查询数据，并插入相关数据表中
    @Insert({"insert into DCL_IO_DECL_GOODS_TASK SELECT * FROM DCL_IO_DECL_GOODS ",
    ...
    int batchInsert(@Param("GOODS_NO")BigDecimal GOODS_ID, @Param("DECL_NO") String DECL_NO, @Param("OPER_TIME_TJ") Date OPER_TIME_TJ);


    @Delete({"DELETE FROM DCL_IO_DECL_GOODS_TASK WHERE GOODS_NO=#{GOODS_NO,jdbcType=DECIMAL} AND DECL_NO=#{DECL_NO,jdbcType=VARCHAR}"})
    int deleteByGOODS_NOAndDECL_NO(@Param("GOODS_NO") BigDecimal GOODS_NO,@Param("DECL_NO") String DECL_NO);

    @Delete({"DELETE FROM DCL_IO_DECL_GOODS_TASK"})
    int deleteAll();
