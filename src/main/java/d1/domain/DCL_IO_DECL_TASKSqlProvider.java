...
public class DCL_IO_DECL_TASKSqlProvider {

    //批量删除
    public String batchDelete(@Param("lists") List<DCL_IO_DECL_TASK> lists) {
        //DCL_IO_DECL_ID为主键
        StringBuilder sb = new StringBuilder();
        sb.append("DELETE FROM DCL_IO_DECL_TASK WHERE DCL_IO_DECL_ID in (");
        MessageFormat messageFormat = new MessageFormat("#'{'lists[{0}].DCL_IO_DECL_ID}");
    ...
    public String insertSelective(DCL_IO_DECL_TASK record) {
        SQL sql = new SQL();
        sql.INSERT_INTO("DCL_IO_DECL_TASK");
