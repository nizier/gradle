...
@Component
@Mapper
public interface INS_RESULT_SUM_TASKMapper {
    @Delete({
        "delete from INS_RESULT_SUM_TASK",
        "where DECL_NO=#{DECL_NO,jdbcType =VARCHAR}"
    })
    int deleteByDECL_NO(String DECL_NO);

    @Delete({"delete from INS_RESULT_SUM_TASK"})
    int deleteAll();

    @Insert({
        "insert into INS_RESULT_SUM_TASK (PRI_KEY, DECL_NO, ",
    ...
    int insert(INS_RESULT_SUM_TASK record);

    @InsertProvider(type=INS_RESULT_SUM_TASKSqlProvider.class, method="insertSelective")
    int insertSelective(INS_RESULT_SUM_TASK record);
    ...
    @Select({
        ...
        "TRANS_TYPE_TJ, OPER_TIME_TJ, IsCheck",
        "from INS_RESULT_SUM_TASK",
        "where PRI_KEY = #{priKey,jdbcType=VARCHAR}"
    })
    @Results(id="INS_RESULT_SUM_TASK_RESULTMAP",value={
        ...
        @Result(column="IsCheck", property="ischeck", jdbcType=JdbcType.DECIMAL)
    })
    INS_RESULT_SUM_TASK selectByPrimaryKey(String priKey);

    @UpdateProvider(type=INS_RESULT_SUM_TASKSqlProvider.class, method="updateByPrimaryKeySelective")
    int updateByPrimaryKeySelective(INS_RESULT_SUM_TASK record);

    @Update({
        "update INS_RESULT_SUM_TASK",
    ...
    int updateByPrimaryKey(INS_RESULT_SUM_TASK record);

    @Select("SELECT * FROM INS_RESULT_SUM WHERE \"OPER_TIME_TJ\">=to_date(#{QueryTime,jdbcType=VARCHAR}, 'yyyy-mm-dd') AND \"OPER_TIME_TJ\"<to_date(#{QueryEndTime,jdbcType=VARCHAR}, 'yyyy-mm-dd') ")
    @ResultMap("INS_RESULT_SUM_TASK_RESULTMAP")
    List<INS_RESULT_SUM_TASK> listDatas(@Param("QueryTime") String QueryTime,@Param("QueryEndTime")String QueryEndTime);

    // OPER_TIME_TJ
    @Select({"SELECT DECL_NO,MAX(OPER_TIME_TJ) AS \"OPER_TIME_TJ\"FROM INS_RESULT_SUM WHERE ROWNUM<10000 GROUP BY DECL_NO"})
    @Results({
    ...
    List<INS_RESULT_SUM_TEMP> listTempDatas();

    //根据报检号及OPER_TIME_TJ，查询数据，并插入相关的任务表中
    @Insert({"insert into INS_RESULT_SUM_TASK SELECT * FROM INS_RESULT_SUM ",
    ...
    @InsertProvider(type=INS_RESULT_SUM_TASKSqlProvider.class, method="batchDelete")
    int batchDelete(@Param("lists") List<INS_RESULT_SUM_TASK> lists);
}